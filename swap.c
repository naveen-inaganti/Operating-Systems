...
{ 
sem_wait(&disk_mutex);
int location, ret, retsize, k,frameno,addr,frameoff=0;
...
/*void signal_semaphore(int count)
{int i;
for(i=0;i<count;i++){
sem_post(&swap_semaq);
}
...
int ret;
  ...
  else printf ("swapQ thread has been created successsfully\n");
